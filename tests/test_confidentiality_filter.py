"""
test_confidentiality_filter.py
...
def test_strict_mode_raises_violation() -> None:
    ...
    # A string that fails validation even after sanitization attempt
    raw_too_long = " ".join(["word"] * 30) + " — sync licensing status"
    # Even if strict, if it can sanitize it will; if dirty terms remain, it raises
    with pytest.raises(ConfidentialityViolationError):
        # Force a case with persistent violations by monkeypatching or unresolvable content
