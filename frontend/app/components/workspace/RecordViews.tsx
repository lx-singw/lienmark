...
import { productionName, organizationName, roleLabel, savedDate, statusLabel } from './labels';
import { PageHeading, PanelTitle, EmptyState, SampleNotice, ReferenceNotice } from './Primitives';
import { ClaimTable } from './ClaimTable';
import { AutomationSettings } from './AutomationView';
...
export function EvidenceView() {
  ...
  const evidence = claims.flatMap(claim => (claim.sources?.length ? claim.sources : claim.evidence ? [claim.evidence] : []).map((source, index) => ({ ...claim, id: `${claim.id}:${index}`, evidence: source }))).filter(claim => `${claim.title} ${claim.evidence.title}`.toLowerCase().includes(query.toLowerCase()));
  return <><PageHeading eyebrow="EVIDENCE LIBRARY" title="A source behind every finding." description="Inspect attributable evidence in the context of the claim it supports." /><WorkspaceNotice />
    ...
    {evidence.length ? <div className="ws-evidence-grid">{evidence.map(claim => <article className="ws-panel ws-evidence-card" key={claim.id}><FileText size={22} /><h3>{claim.evidence!.title}</h3><p>{claim.evidence!.excerpt || 'No excerpt was supplied.'}</p>
      <dl className="ws-definition ws-section-gap"><dt>Related asset</dt><dd>{claim.title}</dd><dt>Provider</dt><dd>{claim.evidence!.provider}</dd><dt>Retrieved</dt><dd>{savedDate(claim.evidence!.retrievedAt)}</dd></dl>
...
export function DecisionsView() {
  ...
  return <><PageHeading eyebrow="REVIEW & DECISIONS" title="Keep every sign-off accountable." description="Review affected claims and inspect recorded decisions without losing their lineage." /><WorkspaceNotice />
    ...
    <div id="review-panel" role="tabpanel" aria-labelledby={`tab-${tab}`}>{tab !== 'history' ? <ClaimTable key={tab} claims={claims.filter(claim => tab === 'preserved' ? ['preserved', 'reviewed'].includes(claim.status) : ['reopened', 'new', 'exception'].includes(claim.status))} title={tab === 'preserved' ? 'Approval continuity' : 'Reviewer queue'} />
      : <section className="ws-panel">{events.length ? <ul className="ws-timeline">{events.map((event, index) => <li key={event.event_id || index}><ShieldCheck size={18} /><div><h3>{statusLabel(event.action)}</h3><p>{event.counsel_rationale || event.rationale || 'No rationale returned.'}</p><small>{event.reviewer_name && !/^(user_|test-|local-verification)/.test(event.reviewer_name) ? event.reviewer_name : 'Production reviewer'} · {savedDate(event.timestamp)}</small><details className="ws-disclosure"><summary>Decision references</summary><dl className="ws-definition ws-section-gap"><dt>Reviewer reference</dt><dd>{event.reviewer_name || 'Not supplied'}</dd><dt>Asset reference</dt><dd>{event.stable_lineage_key}</dd><dt>Prior decision</dt><dd>{event.prior_decision_id || 'Not supplied'}</dd><dt>Event digest</dt><dd>{event.event_hash || 'Not supplied'}</dd></dl></details></div></li>)}</ul> : <EmptyState title="No recorded decisions" description="Verified reviewer actions will appear here when returned by the production service." />}</section>}</div></>;
}
...
export function SettingsView() {
  ...
  return <><PageHeading eyebrow="WORKSPACE SETTINGS" title="The right access. The right context." description="Inspect your session and understand how responsibilities are separated." /><WorkspaceNotice />
    <div className="ws-evidence-grid"><section className="ws-panel"><PanelTitle title="Workspace identity" /><div className="ws-panel-body"><dl className="ws-definition"><dt>Member</dt><dd>{user?.display_name || 'Guest'}</dd><dt>Role</dt><dd>{sample ? 'Read-only preview' : roleLabel(user?.role)}</dd><dt>Production</dt><dd>{sample ? 'Fictional sample' : productionName(user)}</dd><dt>Organization</dt><dd>{sample ? 'Not signed in' : organizationName(user)}</dd></dl>{sample && <button className="ws-button ws-section-gap" onClick={() => setAccessOpen(true)}>Request an invitation</button>}</div></section>
    ...
    <AutomationSettings /><section className="ws-panel ws-section-gap"><PanelTitle title="Workspace preferences" /><div className="ws-panel-body"><details className="ws-disclosure"><summary>Access and session recovery</summary><p>Returning members with a valid session open their workspace directly. Request a new invitation if your link is expired or already used.</p></details></div></section></>;
}
