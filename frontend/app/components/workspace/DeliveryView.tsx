...
export default function DeliveryView() {
  ...
  async function download() {
    ...
    try {
      const response = await fetch(`${audit.statusUrl}/pdf`, { credentials: 'same-origin', cache: 'no-store' });
      ...
      const anchor = document.createElement('a'); anchor.href = url; anchor.download = `${productionName(user).replace(/[^a-z0-9]+/gi, '-').toLowerCase()}-exceptions-schedule.pdf`; anchor.click();
