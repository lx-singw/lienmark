 ...
 * ReportLab PDF exceptions schedule export, and statutory underwriting disclaimers.
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            ...
            {auditId && (
              <span className="block text-[11px] text-sky-400 mt-1 font-mono">
                Bound to Audit: {auditId} | Snapshot: {resultSnapshotId || 'snapshot_0'}
              </span>
            )}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          {auditId && resultSnapshotId && (
            <button
              type="button"
              onClick={handleExportPdf}
              className="flex items-center gap-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-xs font-bold text-white transition-all shadow-md active:scale-95 focus:outline-none focus:ring-2 focus:ring-emerald-300"
              aria-label="Export Draft Exceptions Schedule (PDF)"
            >
              <FileCheck className="h-4 w-4" aria-hidden="true" />
              <span>Export Draft Exceptions Schedule (PDF)</span>
            </button>
          )}
