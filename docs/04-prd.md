...
- The **Director** story is the genuine six-agent orchestration (Discovery, Intake, Research, Ledger, Risk Scoring, Report) — a real production-ready network, not a single wrapped prompt
...
- **Dual-Input Architecture**:
  ...
  - **Method B (Manual Web Portal)**: Drag-and-Drop Dropzone (`frontend/app/page.tsx`) enabling freelance screenwriters, external legal counsel, or independent producers to manually upload screenplay files (`.pdf`, `.fdx`, `.fountain`, `.txt`) or edit timelines (`.edl`, `.xml`, `.aaf`) without direct Cloud Storage bucket access.
- Must accept any industry-standard screenplay format (PDF, Final Draft `.fdx`, Fountain plain-text markup `.fountain`, plain text `.txt`), edit timelines (FCP XML, DaVinci EDL, Avid AAF), or video cuts (`.mp4`, `.mov` via Gemini Multimodal Vision) as source input documents.
...
- **Automated Script Delta-Diffing & Revision Colors**: When a revised script draft is ingested, the Intake Agent executes a semantic delta diff (`is_delta_modified: true`) and stamps standard production revision colors (`revision_color: "White" | "Blue" | "Pink" | "Yellow" | "Green" | "Goldenrod" | "Salmon" | "Cherry"`).
- **Dual Music Clearance Licensing Split**: Automatically decomposes music claims into dual sub-items: `composition_publishing_rights` (ASCAP/BMI/ISWC) and `master_recording_rights` (Label/ISRC), preventing unnecessary master recording licensing costs for cover versions.
- **DMCA Section 1201 Anti-Circumvention DRM Guard**: Pre-scans media files (`drm_protected: false`) ensuring Lienmark never attempts to decrypt or bypass DRM controls, guaranteeing 100% Section 1201 legal compliance.
- **SAG-AFTRA Crowd Release vs. Featured Extra Option Split**: Categorizes performers by on-screen prominence (`crowd_background` vs `featured_speaking`), preventing legal teams from chasing individual contracts for anonymous crowd extras.
...
- **SAG-AFTRA Guild Residuals & Option Expiration Tracker**: For actor likenesses, voice doubles, or archival footage, the Intake/Research Agents track guild option expiration dates (`union_option_expires_at`), autonomously alerting production legal 60 days before distribution rights expire in key territories.
...
- **Multimodal Visual IP & Logo Detector**: For video cuts and storyboard inputs, the Intake Agent utilizes Gemini 3.6 Multimodal Vision to detect visible background brand logos (e.g. Nike swoosh on clothing, Coca-Cola sign), extracting frame timecodes and `visual_bounding_box` coordinates for clearance evaluation.
- **FCP XML / DaVinci EDL Timeline Conformer & Frame-Rate Drift Guard**: Parses industry-standard Final Cut Pro XML, DaVinci Resolve EDL, and Avid AAF edit decision lists (`timeline_conformer.py`), explicitly parsing timecode frame rates (`timecode_fps: 23.976_ndf` | `24.0` | `29.97_df`) to eliminate timestamp drift.
...
- **PRO Music Work ID Target Resolution**: Formats Parallel Search API queries to directly resolve global musical work identifiers (ISWC, ISRC, ASCAP Work ID, BMI Work ID), linking web findings to performance rights registries.
- **Tiered Licensing Scoping Engine**: Evaluates claims against production licensing tiers (`licensing_scope: "Festival Rights Only"` vs `"Worldwide All-Media Perpetual"`), preventing indie productions from overpaying for theatrical rights prematurely.
- **Native Parallel MCP Server Integration**: Supports direct JSON-RPC and SSE transport connecting to Parallel's official Model Context Protocol server (`https://search.parallel.ai/mcp` via `parallel_mcp_client.py`), registered natively within Google Cloud Agent Builder via `agent_builder_mcp_config.json`.
...
- **Multi-Jurisdiction Territory Rights Routing & Release Windowing**: For productions with international distribution tags (`territory_codes` e.g. US, EU, UK, JP), the Research Agent constructs territory-specific queries to local rights databases (GEMA in Germany, JASRAC in Japan, SACEM in France, PRS in the UK) and tracks sequential release windows (`window_stage`).
...
- **Industry Licensing Cost Floor & SIR Deductible Exposure Calculator**: Calculates whether an uncleared claim's maximum statutory exposure (`statutory_exposure_max`) falls below or above the production's E&O Self-Insured Retention deductible (`sir_deductible_usd: 25000`), flagging un-insured out-of-pocket studio risk.
- **Autonomous Self-Correction & Reflection Feedback Loop**: If a Parallel search query returns poor-quality findings (`eval_score < 0.70`), the Research Agent executes an internal self-reflection pass (`self_correction_loop.py`), analyzing query failures and autonomously reformulating its search strategy without human intervention (`reflection_attempts: 1+`).
- **Multi-Agent Inter-Agent Negotiation Protocol**: When the Risk Scoring Agent discovers conflicting evidence, it dispatches a targeted negotiation prompt to the Research Agent (`agent_negotiator.py`), requesting specialized secondary verification (`site:copyright.gov`) to resolve the contradiction before finalizing the verdict.
- **Autonomous Circuit Breaker & Fallback Provider Switch**: If an external registry domain experiences network degradation or 5xx errors, the Research Agent autonomously trips a circuit breaker (`circuit_state: open` in `circuit_breaker.py`), switching to cached public mirrors while preserving pipeline liveness.
- **Goal-Driven Sub-Goal Decomposer & Verification Planner**: For complex multi-layered claims (e.g. uncleared sample used in a movie clip), the Research Agent decomposes the clearance goal into sub-goals (`subgoal_planner.py`: composition sync, master recording, sample lineage), validating each sub-goal independently.
- **Autonomous Research Plan Synthesis & Execution Graph**: Before issuing web queries, the Research Agent dynamically generates a structured `query_plan` DAG (`research_planner.py`), logging its step-by-step reasoning tree to Firestore before execution.
- **Autonomous Claim Dependency & Hierarchy Resolver**: Identifies legal dependencies between claims (`claim_dependency_resolver.py` e.g. sample clearance depending on master track rights), dynamically ordering research to resolve prerequisite parent claims (`parent_claim_id`) first.
- **Dynamic Tool Synthesis & Prompt Strategy Adapter**: If standard query templates yield inconclusive results on an unfamiliar domain, the Research Agent dynamically adapts its extraction prompts and schema parameters (`adapted_extraction_schema` in `tool_synthesizer.py`), synthesizing a tailored strategy on the fly.
- **Multi-Agent Peer Deliberation & Consensus Voting**: For catastrophic risk claims (potential $1M+ lawsuit exposure), the system spawns 3 peer evaluator agents (`peer_deliberation.py` - Conservative Counsel, Litigation Defense, Sync Specialist) to deliberate and vote on final risk classification (`peer_vote_consensus: 3/3`).
...
- **Dynamic 95-Year Rolling Public Domain Calculator**: `statutory_rule_engine.py` dynamically evaluates public domain entry (`publication_year <= current_year - 95`), automatically classifying 1930 works as Public Domain in 2026.
- **Jurisdiction-Aware Right of Publicity Evaluator**: Evaluates state-specific post-mortem publicity rights (California 70-year post-mortem under Cal. Civ. Code § 3344.1 vs. New York 40-year cap) for deceased historical figures.
- **First Amendment Docudrama / Biopic Immunity Classifier**: Identifies biographical storytelling contexts (`is_docudrama_context: true`), tagging non-endorsed historical figure depictions as First Amendment Protected Speech.
- **Trademark Nominative Use vs. Brand Disparagement Classifier**: Distinguishes neutral background brand placement (Green - Nominative Fair Use) from brand disparagement scenes (Red - Trademark Defamation Risk) based on scene sentiment analysis (`is_brand_disparaged`).
- **3-Second *De Minimis* Visual Prominence Metric**: Evaluates background artwork timecode duration (`aggregate_duration_seconds: <3.0s`) and focus rating (`is_out_of_focus: true`) under *Ringgold* precedent to trigger De Minimis non-infringement classification.
- **Structured 17 U.S.C. § 107 4-Factor Fair Use Scorecard**: Outputs a 4-factor percentage breakdown (Purpose & Character, Nature of Work, Amount Used, Market Harm) rather than a single black-box score.
...
- **Completion Bond Underwriting Contingency Risk Score**: Calculates a `bond_compliance_score` (%) and flags whether uncleared IP risk exceeds completion bond contingency thresholds, preventing unvetted claims from halting production budget drawdowns.
- **Pure Python Statutory Legal Rule Engine**: Codifies explicit statutory legal thresholds (17 U.S.C. § 107 Fair Use 4-factor scoring matrix, 17 U.S.C. § 504 statutory damages caps) into a pure Python evaluation engine (`statutory_rule_engine.py`) that executes zero LLM calls during risk tier calculation (`statutory_rule_eval`), eliminating freehand LLM risk scoring hallucinations entirely.
- **Statutory Damages Exposure Calculator**: Calculates explicit statutory liability ranges under 17 U.S.C. § 504(c) ($750 to $30,000 per innocent infringement, up to $150,000 for willful infringement) for uncleared claims (`statutory_exposure_max` via `statutory_damages_calc.py`), providing counsel with worst-case financial lawsuit exposure metrics.
...
- **30-Day Clearance Expiration & Delta TTL Auditor**: Enforces a `verification_ttl_days: 30` expiration lifecycle on ledger records, triggering automatic re-verification passes when productions enter Picture Lock past 30 days.
...
- **Dual-Key Cryptographic Attorney Signature Engine**: Requires a dual-key digital signature (RSA-256 / Ed25519) from both reviewing attorney and lead legal officer (`attorney_signature_hash` via `dual_key_signer.py`) before any high-risk claim can be marked `attorney_cleared` on the immutable ledger.
- **Attorney Override Rejection & Re-Investigation Loop**: When an attorney clicks "Reject Finding" in `AttorneyOverrideModal.tsx`, the Ledger Agent logs `action_type: attorney_rejection` with specific instructions (`attorney_rejection_directive`), routing the claim back to the Research Agent (`attorney_rejection_router.py`) for targeted re-investigation based on human counsel guidance.
- **Standardized Legal Audit Trail Manifest Exporter**: Generates a standardized **Legal Audit Trail Manifest (ISO 27001 / SOC 2 Legal Compliance)** (`manifest_iso_legal.json` via `legal_audit_exporter.py`), detailing every prompt version, raw Parallel Search API payload, timestamp, and attorney override rationale for every claim.
- **Attorney Ethics & Conflict-of-Interest Pre-Screening**: Automatically checks law firm records (`ethics_pre_screening.py`) before assigning claims in `AttorneyOverrideModal.tsx` to verify counsel has zero conflict of interest (`conflict_free_attorney: true`) under ABA Model Rules.
- **RFC 3161 Trusted Cryptographic Timestamping Anchor**: Periodically anchors Firestore SHA-256 hash chains (`anchor_service.py`) to an RFC 3161 compliant Trusted Timestamping Authority or public L2 network (`blockchain_anchor_tx`), establishing court-admissible proof of time under FRE Rule 902(13).
- **Automated Attorney Defense Memorandum Exporter**: Automatically compiles human attorney sign-offs into a formal **Attorney Defense Memorandum** PDF (`legal_brief_doc.pdf` via `legal_brief_exporter.py`) combining statutory citations, Parallel Search API snippets, and Fair Use analysis ready for court submission.
...
- **Official E&O Title Clearance Certificate Generator**: Generates an official PDF audit certificate — **Lienmark Chain-of-Title Clearance Certificate (Form E&O-2026)** — featuring cryptographic hash verification, itemized clearance lists, and attorney sign-off stamps required by E&O insurers (Chubb, Hiscox) before policy binding.
- **Standardized ASCAP / BMI Music Cue Sheet Exporter**: Exports industry-standard music cue sheets (`cue_sheet_standard_v2.csv` / PDF) mapping music claims to scene timestamps, duration, usage codes (*BI = Background Instrumental, VV = Visual Vocal*), publisher splits, and PRO work IDs.
- **Underwriting Partner API Webhook Integrator**: Exposes a secure carrier API endpoint (`POST /api/v1/underwriting/bind-policy`) allowing E&O insurance underwriters (Chubb, Hiscox) to programmatically pull verified ledger audit certificates and automatically issue policy binders.
- **Post-Production Wrap Delivery Checklist**: Generates a single **Post-Production Wrap Clearance Summary** verifying that 100% of claims are either `attorney_cleared` or covered by executed sync agreements (`wrap_delivery_cleared: true`) before distributors (A24, Netflix) release final funds.
