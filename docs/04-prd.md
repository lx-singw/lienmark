...
### 5.3 Live research & Multi-Tool Selection (Research Agent)
...
- Must execute self-directed multi-hop chained research: if an initial search snippet references a connected rights-holder, subsidiary, or broadcast licensee, the Research Agent autonomously issues follow-up queries chasing the lead rather than stopping after one query pass.
- Must support mid-run claim proposals: if the Research Agent discovers an unextracted secondary claim during web verification, it can propose adding the claim to the Intake Agent. Proposed claims must pass full Intake claim-schema validation before being accepted into the pipeline.
